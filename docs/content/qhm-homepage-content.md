# QHM Law Firm — Updated Homepage Content

This file contains the content structure for the updated homepage shown in the provided screenshots. It is written for a zero-content Next.js + Tailwind CSS project, so every visible homepage section is documented as reusable content data.
...
**Brand:** Qaisar H Metawea Law Firm
...
```txt
Independent. Saudi-based. Internationally experienced.
```
...
## 3. Managing Partner Feature Card

> Note: The client previously requested removing the old “Meet Dr Qaisar” section from the homepage. In this updated design, the content appears as a premium profile card, not as a large standalone biography section.
...
```txt
Use a full-width background image of a premium office/interior environment.
Apply a white or warm overlay to keep text readable.
The left side contains the heading and intro.
The right side contains a large numbered sector list with thin dividers and circular icons.
```

---

## 7. Representative Mandates Section
...
## 8. Corporate Enquiry Form Section
...
```ts
export const corporateEnquirySubmit = {
...
## 11. Recommended Homepage Section Order
...
## 12. Complete Homepage Content Object
...
Do not hardcode long text directly inside components.
